...
| --- | --- | --- | --- | --- |
| CoinMarketCap | Skill Hub `find_skill` / `execute_skill` signal layer | `apps/agent/src/cmc/skillHubClient.ts`, `apps/agent/src/cmc/normalizeSignals.ts` | `artifacts/demo-run-latest.json` CMC section | Mock/file mode ready; native MCP waits for local key |
